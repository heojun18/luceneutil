...
final class BlockMaxConjunctionScorer extends Scorer {
  ...
  BlockMaxConjunctionScorer(Weight weight, Collection<Scorer> scorersList) throws IOException {
    super(weight);
		...
		System.out.println("[arcj] BlockMaxConjunctionScorer::BlockMaxConjunctionScorer");
  ...
  public DocIdSetIterator iterator() {
		...
		System.out.println("[arcj] BlockMaxConjunctionScorer::Iterator");
    ...
    return new DocIdSetIterator() {
      ...
      private void moveToNextBlock(int target) throws IOException {
        ...
        maxScore = getMaxScore(upTo);
				//System.out.println("[arcj] BlockMaxConjunctionScorer::moveToNextBlock1 " + target + " / " + upTo + " / " + maxScore);
        ...
        for (int i = scorers.length - 1; i > 0; --i) {
          ...
          float clauseMaxScore = scorers[i].getMaxScore(upTo);
					System.out.println("[arcj] BlockMaxConjunctionScorer::moveToNextBlock2 " + minScore + " / " + clauseMaxScore);
      ...
      private int doNext(int doc) throws IOException {
				...
				System.out.println("[arcj] BlockMaxConjunctionScorer::DoNext");
        advanceHead: for(;;) {
					System.out.println("[arcj] BlockMaxConjunctionScorer::DoNext InnerFunction: " + doc + " / " + iterators.length + " / " + upTo);
          ...
          if (doc > upTo) {
            ...
            final int nextTarget = advanceTarget(doc);
						System.out.println("[arcj] DoNext InnerFunction1: " + nextTarget + " / " + doc);
            if (nextTarget != doc) {
          ...
          if (minScore > 0) {
            ...
            if (score < minScores[0]) {
              ...
              doc = lead.advance(advanceTarget(doc + 1));
							System.out.println("[arcj] DoNext InnerFunction2: " + advanceTarget(doc + 1) + " / " + doc);
              continue;
            }
          }
          ...
          for (int i = 1; i < iterators.length; ++i) {
            ...
            if (other.docID() < doc) {
              final int next = other.advance(doc);
							System.out.println("[arcj] DoNext InnerFunction3: " + other + " / " + next + " / " + doc);
            ...
            if (minScore > 0) {
              ...
              if (score < minScores[i]) {
                ...
                doc = lead.advance(advanceTarget(doc + 1));
								System.out.println("[arcj] DoNext InnerFunction4: " + score + " / " + advanceTarget(doc + 1) + " / " + doc);
                continue advanceHead;
              }
            }
          }
  ...
  public int advanceShallow(int target) throws IOException {
    ...
    int result = scorers[0].advanceShallow(target);
		System.out.println("[arcj] BlockMaxConjunctionScorer::advanceShallow " + scorers[0] + " / " + scorers.length + " / " + target + " / " + result);
